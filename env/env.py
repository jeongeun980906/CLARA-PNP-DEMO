...
COLORS = {
    ...
    "orange": (250/255, 142/255, 43/255, 255/255),
}
...
class PickPlaceEnv():
  ...
  def reset(self, config):
    ...
    self.robot_id = pybullet.loadURDF("./env/ur5e/ur5e.urdf", [0, 0, 0], flags=pybullet.URDF_USE_MATERIAL_COLORS_FROM_MTL)
    self.ghost_id = pybullet.loadURDF("./env/ur5e/ur5e.urdf", [0, 0, -10])  # For forward kinematics.
    ...
    for obj_name in obj_names:
      ...
      if ("block" in obj_name) or ("bowl" in obj_name):
        # Load object.
        object_color = obj_name.split(" ")[0]
        object_type = obj_name.split(" ")[1]
        object_position = rand_xyz.squeeze()
        if object_color in COLORS.keys():
          object_color = COLORS[object_color]
          if object_type == "block":
            ...
            object_position[2] = 0
            object_id = pybullet.loadURDF("./env/bowl/bowl.urdf", object_position, useFixedBase=1)
          
          pybullet.changeVisualShape(object_id, -1, rgbaColor=object_color)
        else:
          object_position[2] = 0.05
          print(object_position)
          object_id = pybullet.loadURDF("./env/objs/" + object_color +'/'+object_color+ ".urdf", object_position, useFixedBase=False)
      else:
        # print("others")
        object_position = rand_xyz.squeeze()
        object_position[2] = 0.05
        print(object_position)
        object_id = pybullet.loadURDF("./env/objs/" + obj_name +'/'+obj_name+ ".urdf", object_position, useFixedBase=False)
        # print("ID", object_id)
  ...
  def obj_xyz(self,obj_name):
    for _ in range(20):
      pybullet.stepSimulation()
    try:
      target = PLACE_TARGETS[obj_name]
    except:
      target = None
    if target == None:
      obj_id = self.obj_name_to_id[obj_name]
      obj_xyz = np.float32(pybullet.getBasePositionAndOrientation(obj_id)[0])
    else:
      obj_xyz = np.float32(target)
    return obj_xyz
